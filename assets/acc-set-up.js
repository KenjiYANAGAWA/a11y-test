...
var accSetUp = () => {
  ...
  try {
    ...
    listIssue.innerHTML = `
      <h2 tabindex='0'>${issues.length == 1 ? 'No' : issues.length - 2} issue${(issues.length - 2) > 1 || issues.length == 1 ? 's' : ''} on this page. CTRL + i to close.</h2>
      ${issues.join('\n')}
  ...
  if (location.pathname == '/pages/checkout') {
    ...
    document.querySelectorAll('[aria-label="Shipping method"] .fieldset-item input[type=radio]').forEach((input) => {
